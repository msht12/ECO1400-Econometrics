...
# change math (incorrect) to math adjusted (correct) and 
# verbal (incorrect) to verbal adjusted (correct)
cfps$math = cfps$math_adj
cfps$verbal = cfps$verbal_adj
...
# --- Summary statistics ---
sum_stats = select(cfps_final, c('math','verbal','total_salary','urban','divorced_dummy',
                         'total_fam_tut_hrs','educ_years_m','educ_years_f','age'))
sum_stats_table = describe(sum_stats, fast=TRUE)
xtable(sum_stats_table)
```
...
reg_pooled = plm(math ~  total_salary + urban
...
reg_pooled_16to18 = plm(math ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_16to18, index=c("pid", "intyear"),
           model="pooling", effect="twoways")

reg_pooled_14to16 = plm(math ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_14to16, index=c("pid", "intyear"),
           model="pooling", effect="twoways")

reg_pooled_12to14 = plm(math ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_12to14, index=c("pid", "intyear"),
           model="pooling", effect="twoways")

reg_pooled_10to12 = plm(math ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
...
reg_pooled = plm(math ~  total_salary + total_salary^2 + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_final, index=c("pid", "intyear"),
           model="pooling", effect="twoways")
...
reg_pooled_16to18 = plm(math ~  total_salary + total_salary^2 + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_16to18, index=c("pid", "intyear"),
           model="pooling", effect="twoways")

reg_pooled_14to16 = plm(math ~  total_salary + total_salary^2 + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_14to16, index=c("pid", "intyear"),
           model="pooling", effect="twoways")

reg_pooled_12to14 = plm(math ~  total_salary + total_salary^2 + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_12to14, index=c("pid", "intyear"),
           model="pooling", effect="twoways")

reg_pooled_10to12 = plm(math ~  total_salary + total_salary^2 + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_10to12, index=c("pid", "intyear"),
           model="pooling", effect="twoways")
...
# -------------------------------------------------------------------------
# Note that the step function in R for variable selection using AIC/BIC only works
# on lm and glm objects, plm won't work. Since pooled reg is equivalent to regular
# OLS, use lm function instead of plm to run pooled regressions.

# --- pooled reg with full dataset ---
reg_pooled = lm(math ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_final)

# variable selection using AIC
sel.var.aic <- step(object=reg_pooled, trace = 0, k = 2, direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled))
...
n=nrow(cfps_final)
sel.var.bic <- step(object=reg_pooled, trace = 0, k = log(n), direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled)) 
select.var.bic<-attr(terms(sel.var.bic), "term.labels")   
select.var.bic

# run regressions according to AIC and BIC
reg_pooled_aic = lm(math ~ urban + divorced_dummy 
            + total_fam_tut_hrs + educ_years_m + educ_years_f, data=cfps_final)

reg_pooled_bic = lm(math ~ urban + divorced_dummy
           + total_fam_tut_hrs + educ_years_m + educ_years_f, data=cfps_final)

# store results in Stargazer table (start with full model as a baseline)
stargazer(reg_pooled, reg_pooled_aic, reg_pooled_bic)
# -------------------------------------------------------------------------

# --- pooled reg with 2016/2018 dataset ---
reg_pooled_16to18 = lm(math ~  total_salary + urban + board_school
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_16to18)

# variable selection using AIC
sel.var.aic <- step(object=reg_pooled_16to18, trace = 0, k = 2, direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled_16to18))
select_var_aic <- attr(terms(sel.var.aic), "term.labels")   
select_var_aic
...
# variable selection using BIC
n=nrow(cfps_final)
sel.var.bic <- step(object=reg_pooled_16to18, trace = 0, k = log(n), direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled_16to18)) 
select.var.bic<-attr(terms(sel.var.bic), "term.labels")   
select.var.bic
# -------------------------------------------------------------------------

# --- pooled reg with 2014/2016 dataset ---
reg_pooled_14to16 = lm(math ~  total_salary + urban + board_school
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_14to16)

# variable selection using AIC
sel.var.aic <- step(object=reg_pooled_14to16, trace = 0, k = 2, direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled_14to16))
select_var_aic <- attr(terms(sel.var.aic), "term.labels")   
select_var_aic
...
# variable selection using BIC
n=nrow(cfps_final)
sel.var.bic <- step(object=reg_pooled_14to16, trace = 0, k = log(n), direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled_14to16)) 
select.var.bic<-attr(terms(sel.var.bic), "term.labels")   
select.var.bic
# -------------------------------------------------------------------------

# --- pooled reg with 2012/2014 dataset ---
reg_pooled_12to14 = lm(math ~  total_salary + urban + board_school
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_12to14)

# variable selection using AIC
sel.var.aic <- step(object=reg_pooled_12to14, trace = 0, k = 2, direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled_12to14))
select_var_aic <- attr(terms(sel.var.aic), "term.labels")   
select_var_aic
...
# variable selection using BIC
n=nrow(cfps_final)
sel.var.bic <- step(object=reg_pooled_12to14, trace = 0, k = log(n), direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled_12to14)) 
select.var.bic<-attr(terms(sel.var.bic), "term.labels")   
select.var.bic
# -------------------------------------------------------------------------

# --- pooled reg with 2010/2012 dataset ---
reg_pooled_10to12 = lm(math ~  total_salary + urban + board_school
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_10to12)

# variable selection using AIC
sel.var.aic <- step(object=reg_pooled_10to12, trace = 0, k = 2, direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled_10to12))
select_var_aic <- attr(terms(sel.var.aic), "term.labels")   
select_var_aic
...
# variable selection using BIC
n=nrow(cfps_final)
sel.var.bic <- step(object=reg_pooled_10to12, trace = 0, k = log(n), direction = "both",
                    scope=list(lower=math ~ divorced_dummy, upper=reg_pooled_10to12)) 
select.var.bic<-attr(terms(sel.var.bic), "term.labels")   
select.var.bic
# -------------------------------------------------------------------------
...
reg_pooled = plm(verbal ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_final, index=c("pid", "intyear"),
           model="pooling", effect="twoways")
...
reg_pooled_16to18 = plm(verbal ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_16to18, index=c("pid", "intyear"),
           model="pooling", effect="twoways")

reg_pooled_14to16 = plm(verbal ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_14to16, index=c("pid", "intyear"),
           model="pooling", effect="twoways")

reg_pooled_12to14 = plm(verbal ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_12to14, index=c("pid", "intyear"),
           model="pooling", effect="twoways")

reg_pooled_10to12 = plm(verbal ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_10to12, index=c("pid", "intyear"),
           model="pooling", effect="twoways")
...
# -------------------------------------------------------------------------
# Note the step function in R for variable selection using AIC/BIC only works
# on lm and glm objects, plm won't work. Since pooled reg is equivalent to regular
# OLS, use lm function instead of plm to run pooled regressions.

# --- pooled reg with full dataset ---
reg_pooled = lm(verbal ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_final)

# variable selection using AIC
sel.var.aic <- step(object=reg_pooled, trace = 0, k = 2, direction = "both",
                    scope=list(lower=verbal ~ divorced_dummy, upper=reg_pooled))
select_var_aic <- attr(terms(sel.var.aic), "term.labels")   
select_var_aic
...
# run regressions according to AIC and BIC
reg_pooled_aic = lm(verbal ~ urban + divorced_dummy 
            + total_fam_tut_hrs + educ_years_m + educ_years_f, data=cfps_final)

reg_pooled_bic = lm(verbal ~  divorced_dummy
           + total_fam_tut_hrs + educ_years_m + educ_years_f, data=cfps_final)
...
FE_twoways_16to18 <- plm(math ~ total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_16to18, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_14to16 <- plm(math ~ total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_14to16, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_12to14 <- plm(math ~ total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_12to14, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_10to12 <- plm(math ~ total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_10to12, index=c("pid", "intyear"),
           model="within", effect="twoways")
...
FE_twoways_16to18 <- plm(math ~ lag(math) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_16to18, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_14to16 <- plm(math ~ lag(math) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_14to16, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_12to14 <- plm(math ~ lag(math) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_12to14, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_10to12 <- plm(math ~ lag(math) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_10to12, index=c("pid", "intyear"),
           model="within", effect="twoways")
...
FE_twoways_16to18 <- plm(verbal ~ total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_16to18, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_14to16 <- plm(verbal ~ total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_14to16, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_12to14 <- plm(verbal ~ total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_12to14, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_10to12 <- plm(verbal ~ total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_10to12, index=c("pid", "intyear"),
           model="within", effect="twoways")
...
FE_twoways_16to18 <- plm(verbal ~ lag(verbal) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_16to18, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_14to16 <- plm(verbal ~ lag(verbal) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_14to16, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_12to14 <- plm(verbal ~ lag(verbal) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_12to14, index=c("pid", "intyear"),
           model="within", effect="twoways")

FE_twoways_10to12 <- plm(verbal ~ lag(verbal) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_10to12, index=c("pid", "intyear"),
           model="within", effect="twoways")
...
FE_twoways_12to18_AB_onestep <- pgmm(math ~ lag(math) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f| lag(math, 2:99),
              data=cfps_12to18, index=c("pid", "intyear"),
              model="onestep", effect="twoways")

FE_twoways_12to18_AB_twostep <- pgmm(math ~ lag(math) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f | lag(math, 2:99),
              data=cfps_12to18, index=c("pid", "intyear"),
              model="twosteps", effect="twoways")

FE_twoways_12to18_sysGMM <- pgmm(math ~ lag(math) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f | lag(math, 2:99),
             data=cfps_12to18, index=c("pid", "intyear"),
             model="twosteps", effect="twoways", transformation = "ld")
...
FE_twoways_12to18_AB_onestep <- pgmm(verbal ~ lag(verbal) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f| lag(verbal, 2:99),
              data=cfps_12to18, index=c("pid", "intyear"),
              model="onestep", effect="twoways")

FE_twoways_12to18_AB_twostep <- pgmm(verbal ~ lag(verbal) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f | lag(verbal, 2:99),
              data=cfps_12to18, index=c("pid", "intyear"),
              model="twosteps", effect="twoways")

FE_twoways_12to18_sysGMM <- pgmm(verbal ~ lag(verbal) + total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f | lag(verbal, 2:99),
             data=cfps_12to18, index=c("pid", "intyear"),
             model="twosteps", effect="twoways", transformation = "ld")
...
logit_alldata = glm(divorced_dummy ~ total_salary + urban
...
reg_pooled_prop_match = lm(math ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_final_prop_match)

reg_pooled_prop_match_verbal = lm(verbal ~  total_salary + urban
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
           data=cfps_final_prop_match)

stargazer(reg_pooled_prop_match, reg_pooled_prop_match_verbal)
# -------------------------------------------------------------------------

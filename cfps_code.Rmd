...
# regressions using 2-year windows
...
FE_time_16to18 <- plm(math ~ lag(math) + total_salary + urban + board_school
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_16to18, index=c("pid", "intyear"),
           model="within", effect="time")

FE_time_14to16 <- plm(math ~ lag(math) + total_salary + urban + board_school
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_14to16, index=c("pid", "intyear"),
           model="within", effect="time")

FE_time_12to14 <- plm(math ~ lag(math) + total_salary + urban + board_school
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_12to14, index=c("pid", "intyear"),
           model="within", effect="time")

FE_time_10to12 <- plm(math ~ lag(math) + total_salary + urban + board_school
            + divorced_dummy + total_fam_tut_hrs + educ_years_m + educ_years_f,
          data = cfps_10to12, index=c("pid", "intyear"),
           model="within", effect="time")

# store results in Stargazer table
stargazer(reg_pooled, reg_pooled_16to18, reg_pooled_14to16, reg_pooled_12to14, 
          reg_pooled_10to12, column.sep.width = "1pt")

stargazer(FE_time_16to18, FE_time_14to16, FE_time_12to14, FE_time_10to12,
